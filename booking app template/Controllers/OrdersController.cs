    ...
    public class OrdersController : Controller
    {
        ...
        public IActionResult ShoppingCart()
        {
            ...
            var response = new ShoppingCartVM()
            {
                ...
                ShoppingCartTotal = (float)_shoppingCart.GetShoppingCartTotal()
            };

            return View(response);
        }

        public async Task<IActionResult> AddItemToShoppingCart(int id, DateOnly datepicker)
        {
            var item = await _productService.GetProductByIdAsync(id);

            if (item != null)
            {
                _shoppingCart.AddItemToCart(item, datepicker);
            }
